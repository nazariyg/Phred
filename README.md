...
It is true, however, that PHP is easy to start with thanks to the C-like syntax resembling JavaScript in addition to a good number of other popular languages and that PHP has become the lingua franca of web programming, with a huge community of developers and open-source contributors. But it's also true that PHP is slowly but surely losing in quality to a bunch of rival languages. Because PHP is going to stay around for many years to come, the demands from developers for an up-to-date PHP standard can no longer be ignored.
