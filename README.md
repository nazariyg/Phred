...
Still largely procedural and hardly following any conventions in the naming of the daunting number of its functions, some of which starting with "str" and some with "str_", with the order of "haystack", "needle", and "subject" parameters changing irregularly from one function to another, and without the long overdue support for Unicode, PHP has found itself in a weird relationship with the developers, where people's love for PHP is mixed with a nearly proportional amount of hate.

It is true, however, that PHP is easy to start with thanks to the C-like syntax resembling JavaScript in addition to a good number of other popular languages as well as the fact that PHP has become the lingua franca of web programming with a huge community of developers and open-source contributors, but it's also true that PHP is slowly but surely losing in quality to a bunch of rival languages. Since PHP is going to stay around for many years to come, the demands from developers for an up-to-date PHP standard can no longer be ignored.
