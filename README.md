...
Phred strives to transform PHP into a cutting-edge, fully-featured, and easy-to-use tool for contemporary web development, while remaining free for all and well-documented. And Phred is many months of work that has already been done and that still needs to be accomplished. *You can help Phred carry on and improve its quality by making a [dontation](http://htmlpreview.github.com/?https://github.com/nazariyg/Phred/blob/master/readme-files/donate.html).*
...
* [Preface](README.md#preface)
    * [PHP 6 or Uniphant vs. Elecorn](README.md#php-6-or-uniphant-vs-elecorn)
    * [PHP 7 or Same as PHP 5.6+ but Faster](README.md#php-7-or-same-as-php-56-but-faster)
    * [こんにちは, Phred](README.md#こんにちは-phred)
    * [The Zen of Phred](README.md#the-zen-of-phred)
* [Installation](README.md#installation)
    * [Getting the Latest PHP Version](README.md#getting-the-latest-php-version)
    * [Installing Phred](README.md#installing-phred)
    * [Running Unit Tests](README.md#running-unit-tests)
* [Fundamental Types](README.md#fundamental-types)
    * [OOP String](README.md#oop-string)
        * [Regular Expressions](README.md#regular-expressions)
        * [Character Encodings](README.md#character-encodings)
    * [OOP Array](README.md#oop-array)
    * [OOP Map](README.md#oop-map)
    * [Time and Time Zones](README.md#time-and-time-zones)
    * [Comparison and Sorting](README.md#comparison-and-sorting)
* [Enhanced Debugging](README.md#enhanced-debugging)
    * [Semantic Checks](README.md#semantic-checks)
    * [Bug Tracking and Reporting](README.md#bug-tracking-and-reporting)
* [U14n](README.md#u14n)
    * [Locales](README.md#locales)
    * [Locale-Specific Formatting](README.md#locale-specific-formatting)
* [Batteries Included](README.md#batteries-included)
    * [Input Filtering](README.md#input-filtering)
    * [Files and Directories](README.md#files-and-directories)
    * [Omnivorous JSON](README.md#omnivorous-json)
    * [Mailing](README.md#mailing)
    * [Requesting](README.md#requesting)
* [Backward Compatibility](README.md#backward-compatibility)
* [Phred's Roadmap](README.md#phreds-roadmap)
...
If you are a web developer, you are most likely familiar with JavaScript. Being highly object-oriented and with Unicode done right, JavaScript just feels comfortable to work with and has recently made it to the server side with Node.js. Same kind of sentiments may go to other languages that, unlike PHP, were designed with a vision in mind, such as Python, Ruby, Scala and others.

While the quality standards for web development have evolved over the past decade to match the increasing demand for clean, readable, and comprehensible code that can be efficiently shared with other people on the team or made public, PHP remained mostly immature. This is especially noticeable if you are a PHP developer working on a modern web application that is required to offer a substantial value to many users around the world. It even seems bit unfair that PHP, which is the most popular language used on the server side with a share of about [80%](http://w3techs.com/technologies/history_overview/programming_language/ms/y) among all web sites in 2014, is not superior to JavaScript, even though the control flow of a web application could imply the opposite.

Still being largely procedural and hardly having following any conventions in the naming of the daunting number of its functions, some of which starting with "str" and some with "str_", with the order of "haystack", "needle", and "subject" parameters changing irregularly from one function to another, and without long overdue support for Unicode, PHP has found itself in a weird relationship with the developers, in which people's love for PHP is mixed with a nearly proportional amount of hate.

It is true that PHP is easy to start with thanks to the C-like syntax resembling JavaScript in addition to a good number of other popular languages as well as the fact that PHP has become the lingua franca of web programming with a huge community of developers and open-source contributors, but it's also true that PHP is gradually losing in quality to a bunch of other languages. Since PHP is here to stay for many years to come, the demands from developers for an up-to-date development tool embodied in PHP can no longer be ignored.

## PHP 6 or Uniphant vs. Elecorn

Most of the hopes for a better PHP were being put in PHP 6, which was announced several year ago. However, PHP 6 over the time became more like a mythical creature that no one has ever seen. The new features planned for PHP along with highly anticipated Unicode support never seen a release date as PHP 6. Instead, the PHP 6 branch was discontinued and only some of the features were backported into the 5.x versions afterwards. There is no beta or even alpha version of PHP 6.

One of the main reasons for PHP 6 getting abandoned was a disagreement inside the development team as to which encoding would serve best for storing and processing Unicode strings. Despite of the fact that the team eventually picked UTF-16 as the internal encoding for Unicode in PHP 6, the development ran out of steam and finally came to a halt. This happened not only because the choice of UTF-16 turned out to be suboptimal, but also because of the immense body of work that was required to be done for the PHP's core and all the extensions by the developers getting less and less enthusiastic with the direction chosen and due to other issues. Andrei Zmievski, who was the head of the PHP 6 project, later admitted that he would probably choose UTF-8 over UTF-16 if it was possible to start over.

## PHP 7 or Same as PHP 5.6+ but Faster

In July of 2014, a vote was held by the PHP's steering group to decide under what name the next major version of PHP should be presented to the public when its development branch, called PHPNG ("PHP New Generation"), would become a release candidate. The name that outvoted "PHP 6" and the rest variants was "PHP 7". Skipping over "6" in the PHP's version was probably for the reason that PHP 6 had bit too much of disappointment associated with it and a number of books already existed with "PHP 6" in their titles. Sadly, PHP 7 adds little changes in the coding standard compared to the latest release in the 5.x series, except in one important aspect, which is performance.

With the advent of PHP 7, the mainstream implementation of PHP (called Zend Engine) made a substantial progress in speed and memory consumption thanks to a great deal of optimization and code refactoring that eliminated numerous bottlenecks in the PHP's performance, while not breaking any backward compatibility. The modified engine also brought PHP closer to being able to benefit from just-in-time compilation in some of its future versions. The benchmarks run with PHP 7 showed surprisingly good results, indicating almost double increase in speed and much smaller memory footprint.

PHP 7 has definitely got some exciting news for web users, not so much for developers. Rasmus Lerdorf, one of the PHP's creators, has reportedly said multiple times that he prefers procedural code over object-oriented one, but this viewpoint is hardly shared by the developer community. PHP developers, who are put in the context of present-day quality standards of coding, are the actual end-users of the product called "PHP", and saying that the customer is always right may really be appropriate here.

## こんにちは, Phred

The idea of Phred is related the principle of separation of concerns: let the PHP's core developers focus on the PHP's underpinnings and further improve the core language to make it even faster and to provide a high-performance foundation upon which a consistent and completely object-oriented coding standard is implemented by PHP developers in their own right.
...
Converting from one character encoding to another and other operations related to character encodings are taken care of by the [CEString](http://htmlpreview.github.com/?https://github.com/nazariyg/Phred/blob/master/doc/classes/CEString.html) class.
